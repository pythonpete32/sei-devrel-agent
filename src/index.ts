...
async function handleTransaction(txHash: string) {
  try {
    ...
    const report = await debugCommands.debugTransaction(validTxHash)
    debugCommands.displayReport(report)
  } catch (error) {
    cli.displayError(error)
  }
...
async function handleParallelExecution(txHash: string) {
  try {
    ...
    const report = await debugCommands.debugParallelExecution(validTxHash)
    debugCommands.displayReport(report)
  } catch (error) {
    cli.displayError(error)
  }
