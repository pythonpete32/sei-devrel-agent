...
4. Optimization opportunities`,
  ...
  private classifyIssue(issue: string, userContext?: string): DebugTask {
    ...
    if (
      lowerIssue.includes('revert') ||
      lowerIssue.includes('failed') ||
      lowerIssue.includes('error')
    ) {
      complexity = 'high'
    } else if (
      lowerIssue.includes('how') ||
      lowerIssue.includes('what') ||
      lowerIssue.includes('status')
    ) {
      complexity = 'low'
    }
  ...
  private formatDebugReport(
    ...
    return {
      analysis: report.content.map((c) => ({
        type: c.type,
        text: c.text,
        citations: c.citations === null ? undefined : c.citations,
      })),
  ...
  private extractRootCause(content: Array<{ type: string; text?: string }>): string | undefined {
